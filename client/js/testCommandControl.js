...
// import {Player,Action,Position,StateHandler} from "./../../server/src/rooms/StateHandler";
var name;
var players = {}
var canvas;
var engine;
...
function main() {
  ...
  client.joinOrCreate("game", {name:name}).then(rooms => {
...
// }); 
  ...
  window.addEventListener("keypress", function(e) {
    ...
    alert("error joining session. Refresh and try again");
  });
...
function loadPlayer(scene, engine, canvas,sessId) {
  ...
  BABYLON.SceneLoader.ImportMesh("", "player/", "Vincent-frontFacing.babylon", scene, function (meshes, particleSystems, skeletons) {
    ...
    cmds = [cc.walk, cc.walkBack, cc.run, cc.jump, cc.turnLeft, cc.turnRight];
    // cc._avatar._position.x = sessObj.position.x;
    canvas.focus();
    resolve(cc);
    // return cc;
  });
